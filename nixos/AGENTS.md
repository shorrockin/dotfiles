...
Flake, the `gustave` host, system modules, Home Manager, and the raw-Hyprland desktop stack (NixOS owns this layer directly; Omarchy has its own Lua/Quickshell equivalent — see `../omarchy/AGENTS.md`). Nothing here applies on macOS or Omarchy.
...
**Never run `nixos-rebuild` directly** — it needs a sudo password the coding agent cannot provide. Make the config changes, then ask the user to run:
```bash
...
`common/config/scripts/dots` pre-creates `~/.config/nvim/{lua,lua/lsp,lua/plugins}` and `~/.config/nvim/after/{plugin,ftplugin}` before stowing. See the shared configuration instructions for details.
